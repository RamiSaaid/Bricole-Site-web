...
<div class="job-container" id="job-listings">
    ...
    if (!empty($budget_order)) {
        ...
        $query .= " ORDER BY FIELD(status, 'Travail terminé'), budget ASC";  // Custom order by status
    }
            ...
            if ($_SESSION['user_type'] == 'pro' && $current_status != "Travail terminé") {
                ...
                if ($confirmed) {
                    if ($confirming_user_id == $pro_user_id) {
                        ...
                        echo "<div style='margin-top: 20px; margin-bottom: 20px; text-align: center; color: white; background-color: #ef7900; padding: 20px;'>";
                        echo "<p>Merci pour votre confirmation. Pour plus de détails sur l'annonce, contactez le particulier. Si vous avez des questions supplémentaires, contactez le support.</p>";
                        echo "</div>";

                        // Display call and send message buttons
                        echo "<div style='margin-top: 20px; margin-bottom: 20px; text-align: center;''>";
                        echo "<a href='tel:" . htmlspecialchars($job['phone']) . "' style='margin-right: 20px;'><i class='fas fa-phone'></i> Appeler le particulier </a>";
                        echo "<a href='mailto:" . htmlspecialchars($job['email']) . "'><i class='fas fa-envelope'></i> Email le particulier </a>";
                        echo "</div>";
                    } else {
                        echo "<hr style='border: 1px solid white; margin-top: 35px;'>";
                        echo "<div style='margin-top: 20px; margin-bottom: 20px; text-align: center; color: white; background-color: #ef7900; padding: 20px;'>";
